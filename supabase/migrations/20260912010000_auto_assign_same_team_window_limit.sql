drop function if exists public.run_organization_auto_assign(uuid,date,date);

create or replace function public.run_organization_auto_assign(
  p_organization_id uuid,
  p_start_date date,
  p_end_date date,
  p_same_team_limit integer default 1
)
returns jsonb
language plpgsql
security invoker
...
revoke all on function public.run_organization_auto_assign(uuid,date,date,integer) from public,anon;
grant execute on function public.run_organization_auto_assign(uuid,date,date,integer) to authenticated;

comment on function public.run_organization_auto_assign(uuid,date,date,integer) is
  'Runs AutoAssign for one organization and caps each official/team pairing within the selected run window.';
