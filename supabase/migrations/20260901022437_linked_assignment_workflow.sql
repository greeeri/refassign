...
as $$
declare
  v_group_id uuid; v_source_order integer; v_target_position uuid;
...
begin
  if not public.can_manage_game_setup() then
    raise exception 'Only Administrators and Assignors can manage assignments';
  end if;
  if p_official_id is null then raise exception 'An official is required'; end if;
  select sort_order into v_source_order from public.sport_positions where id=p_position_id;
  if v_source_order is null then raise exception 'Assignment position not found'; end if;
  select group_id into v_group_id from public.game_link_members where game_id=p_game_id;

  for v_game in
    select g.id,g.sport_id,g.officials_needed from public.games g
    where g.id=p_game_id or (v_group_id is not null and g.id in
      (select glm.game_id from public.game_link_members glm where glm.group_id=v_group_id))
    order by g.starts_at,g.id
  loop
    select sp.id into v_target_position from public.sport_positions sp
    where sp.sport_id=v_game.sport_id order by sp.sort_order,sp.id
    offset v_source_order limit 1;
    if v_target_position is null or v_source_order>=greatest(v_game.officials_needed,0) then
      raise exception 'A linked game does not have the matching assignment position';
    end if;
    select id,position_id into v_official_assignment,v_existing_position
      from public.assignments where game_id=v_game.id and official_id=p_official_id for update;
    ...
    if v_official_assignment is not null and v_existing_position is distinct from v_target_position then
      raise exception 'The official is already assigned to a different position on linked Game %',v_game.id;
    end if;
    if v_existing_assignment is not null then
      update public.assignments set official_id=p_official_id,status='proposed',
        published_at=null,accept_by=null,published_by=null,email_sent_at=null,
        resend_email_id=null,email_error=null,response_token=null,responded_at=null,
        decline_reason=null where id=v_existing_assignment;
...
$$;
revoke all on function public.assign_official_to_linked_games(uuid,uuid,uuid) from public,anon;
grant execute on function public.assign_official_to_linked_games(uuid,uuid,uuid) to authenticated;
...
begin
  ...
  select a.position_id,g.sport_id,g.officials_needed,sp.sort_order into v_source
  from public.assignments a join public.games g on g.id=a.game_id
  ...
  ) positions where positions.slot=v_source.sort_order+p_direction;
  if v_target_position is null then raise exception 'There is no position in that direction'; end if;
  select id into v_target_assignment from public.assignments
    where game_id=p_game_id and position_id=v_target_position and status<>'declined'
    order by assigned_at desc limit 1 for update;
...
$$;
revoke all on function public.move_assignment_position(uuid,uuid,integer) from public,anon;
grant execute on function public.move_assignment_position(uuid,uuid,integer) to authenticated;
