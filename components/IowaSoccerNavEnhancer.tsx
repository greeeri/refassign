...
export default function IowaSoccerNavEnhancer(){
  useEffect(()=>{
    const apply=()=>{
      ...
      if(!nav || nav.querySelector(".iowaSoccerNavGroup")) return;
    ...
    apply(); const obs=new MutationObserver(apply); const nav=document.querySelector("#primary-navigation nav"); if(nav) obs.observe(nav,{childList:true}); return()=>obs.disconnect();
  },[]);
  return null;
}
