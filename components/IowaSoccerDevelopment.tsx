...
type Module = {
  ...
  required: boolean;
  sort_order: number;
  delivery_type: "self_led" | "virtual" | "in_person";
...
export default function IowaSoccerDevelopment() {
  const supabase = useMemo(() => createClient(), []),
    ...
    [registrations, setRegistrations] = useState<Record<string, Registration>>(
      {},
    ),
  ...
  async function load() {
    ...
    if (!user) return;
    const { data: official, error: oe } = await supabase
      .from("officials")
      .select("id")
    ...
    if (oe || !official)
      return setError(
        "Your official account is not connected to the Iowa Soccer development program.",
      );
    ...
    const [m, p, r, q, f] = await Promise.all([
      supabase
      ...
      supabase
        ...
        .select(
          "id,question,status,response,responder_name,asked_at,responded_at",
        )
        .eq("official_id", official.id)
    ...
    const e = m.error || p.error || r.error || q.error || f.error;
    if (e) return setError(e.message);
    setModules((m.data || []) as Module[]);
    const pm: Record<string, Progress> = {};
    for (const row of (p.data || []) as Progress[]) pm[row.module_id] = row;
    setProgress(pm);
    const rm: Record<string, Registration> = {};
    for (const row of (r.data || []) as Registration[]) rm[row.module_id] = row;
    setRegistrations(rm);
  ...
  async function register(m: Module) {
    ...
    setBusy(`reg-${m.id}`);
    setError("");
    const auto = m.delivery_type === "self_led",
      paymentStatus = m.payment_required ? "pending" : "not_required";
    const { error: e } = await supabase
      .from("development_training_registrations")
      .upsert(
        {
        ...
        { onConflict: "module_id,official_id" },
      );
    if (e) setError(e.message);
    else {
      ...
      } else if (m.payment_required && m.payment_url)
        window.open(m.payment_url, "_blank");
      else if (m.registration_url) window.open(m.registration_url, "_blank");
    }
    setBusy("");
  }
  ...
  function deliveryLabel(m: Module) {
    return m.content_type === "quiz"
      ? "Online Test"
      : m.delivery_type === "in_person"
        ? "In Person"
        : m.delivery_type === "virtual"
          ? "Virtual"
          : "Self-Led";
  }
  function schedule(m: Module) {
    if (m.delivery_type === "self_led" || !m.course_start_at) return null;
    const start = new Date(m.course_start_at),
      end = m.course_end_at ? new Date(m.course_end_at) : null;
  ...
  function action(m: Module) {
    ...
    if (reg?.status === "pending")
      return (
        <>
          {m.payment_required && m.payment_url && (
            <a
              className="trainingAction"
              href={m.payment_url}
              target="_blank"
          ...
          <span className="trainingStatus">Registration Pending</span>
        </>
      );
    return (
      <button
        className="trainingAction"
        disabled={busy === `reg-${m.id}`}
    ...
    return (
      <article className="trainingModule" key={m.id}>
        ...
        <div className="trainingMeta">
          <b>{deliveryLabel(m)}</b> • {m.category}
          {m.payment_required ? " • Paid Course" : ""}
        </div>
        {when && (
          <div className="trainingMeta">
            <b>📅 {when}</b>
  ...
  return (
    <div className="iowaTrainingPage">
      <section className="iowaTrainingTitle">
        <div>
          ...
          <p>
            ...
            Be your best on the field.
          </p>
        </div>
        <img
          className="iowaTrainingLogo"
          src="https://cdn.creativeclaw.co/u/6b3cdfa9/images/a48e112e-d080-42b5-8b17-2fcb4ea7e2b3.png"
          alt="Iowa Soccer Referee Development Program — Learn Grow Excel"
        />
      </section>
      <IowaTrainingSupportActions />
      ...
      <section className="trainingSummary">
        ...
        <article>
          <h3>◎ &nbsp; Next Recommended Training</h3>
          {next ? (
            <>{moduleCard(next)}</>
          ) : (
            <p>All available training is complete.</p>
          )}
        </article>
        <article>
          <h3>●● &nbsp; Recent Feedback</h3>
          {questions[0]?.response ? (
            <div className="trainingFeedback">
              “{questions[0].response}”<br />
              <small>
                — {questions[0].responder_name || "Iowa Soccer Mentor"}
              </small>
            </div>
          ) : (
            <p>No mentor feedback has been shared yet.</p>
          )}
        </article>
      </section>
      ...
      <section className="trainingSection">
        <div className="trainingSectionHead">
          <div>
            ...
            <p>Explore additional training resources and materials.</p>
          </div>
          <span>
            <input
              className="trainingSearch"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search training library…"
            />{" "}
            <select
              value={category}
          ...
          {categories.map((c) => (
            ...
            >
              ...
              <small style={{ display: "block" }}>
                ({modules.filter((m) => m.category === c).length} modules)
              </small>
            </button>
          ))}
        </div>
      ...
      <section className="trainingSection">
        ...
        <div className="tableWrap">
          <table className="trainingCompleted">
            <thead>
              <tr>
                <th>Module</th>
                <th>Type</th>
