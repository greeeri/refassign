...
type Focus =
  ...
  | "imports"
  | "conflicts";
const cards: [Focus, string][] = [
  ["needs", "Games needing officials"],
  ["unconfirmed", "Unconfirmed assignments"],
  ["declined", "Declined assignments"],
  ["hold", "Games on hold"],
  ["cancelled", "Cancelled / rain outs"],
...
export default function DashboardGames({
    ...
    async function load() {
      ...
      if (firstError) setError(firstError.message);
        ...
        const nextGames = (g.data || []) as unknown as Game[],
          gameIds = new Set(nextGames.map((game) => game.id));
        setGames(nextGames);
        setAssignments(
  ...
  for (const list of byOfficial.values()) {
    ...
    for (let i = 0; i < list.length; i++)
      for (let j = i + 1; j < list.length; j++) {
        const first = map.get(list[i].game_id)!,
          second = map.get(list[j].game_id)!,
          end =
            new Date(first.starts_at).getTime() +
            (first.duration_minutes || 110) * 60000,
          start = new Date(second.starts_at).getTime();
        if (start >= end) break;
  ...
  const visible =
    focus === "needs"
      ? needs
      : focus === "unconfirmed"
        ? future.filter((g) => unconfirmedIds.has(g.id))
        : focus === "declined"
          ? future.filter((g) => declinedIds.has(g.id))
          : focus === "hold"
            ? hold
            : focus === "cancelled"
              ? cancelled
  ...
  const today = new Date(),
    ...
    todayGames = games.filter(
      (g) => new Date(g.starts_at).toDateString() === todayKey,
    ),
    todayAssigned = new Set(
      assignments
        .filter((a) => {
          const g = map.get(a.game_id);
          return (
            g &&
            new Date(g.starts_at).toDateString() === todayKey &&
            !["declined", "cancelled"].includes(a.status)
          );
        })
        .map((a) => a.official_id),
    ).size;
  const upcoming = active.slice(0, 6);
  const totalActiveGames = active.length,
    fullyAssigned = active.filter(
      (g) => rows(g).length >= g.officials_needed,
    ).length,
    partial = active.filter(
      (g) => rows(g).length > 0 && rows(g).length < g.officials_needed,
    ).length,
    unassigned = active.filter((g) => rows(g).length === 0).length;
  const pct = (n: number) =>
    totalActiveGames ? Math.round((n / totalActiveGames) * 100) : 0;
  const totalOfficials = officials.length,
    activeOfficials = officials.filter((o) => o.active).length,
    inactiveOfficials = totalOfficials - activeOfficials;
    ...
    <div className="modernDashboard">
      ...
      <div className="dashboardMetrics">
        <div className="dashboardMetric">
          <div className="dashboardMetricIcon">▣</div>
          <div>
            <div className="dashboardMetricValue">{todayGames.length}</div>
            <div className="dashboardMetricLabel">Games Today</div>
          </div>
        </div>
        <div className="dashboardMetric green">
          <div className="dashboardMetricIcon">♟</div>
          <div>
            <div className="dashboardMetricValue">{todayAssigned}</div>
            <div className="dashboardMetricLabel">Officials Assigned Today</div>
          </div>
        </div>
        <div className="dashboardMetric">
          <div className="dashboardMetricIcon">◷</div>
          <div>
            <div className="dashboardMetricValue">{needs.length}</div>
            <div className="dashboardMetricLabel">Games Needing Officials</div>
          </div>
        </div>
        <div className="dashboardMetric green">
          <div className="dashboardMetricIcon">✓</div>
          <div>
            <div className="dashboardMetricValue">{unconfirmed.length}</div>
            <div className="dashboardMetricLabel">Awaiting Confirmation</div>
          </div>
        </div>
      </div>
      <div className="dashboardGrid">
        <section className="dashboardPanel">
          <div className="dashboardPanelHead">
            ...
            <button onClick={() => go("Games")}>View All</button>
          </div>
          <div className="tableWrap" style={{ border: 0, borderRadius: 0 }}>
            <table>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={5}>Loading dashboard…</td>
                  </tr>
                ) : upcoming.length ? (
                  upcoming.map((g) => (
                    <tr key={g.id}>
                      ...
                      <td>
                        ...
                        <small>{g.levels?.name || g.leagues?.name || ""}</small>
                      </td>
                      ...
                      <td>
                        <span
                          className={`badge ${rows(g).length >= g.officials_needed ? "green" : "yellow"}`}
                        >
                          {rows(g).length >= g.officials_needed
                            ? "Assigned"
                            : "Open"}
                  ...
                  <tr>
                    <td colSpan={5}>No upcoming games.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <section className="dashboardPanel">
          ...
          <div className="quickActions">
            <button onClick={() => go("Games")}>
              Add / Import Games <span>›</span>
            </button>
            <button onClick={() => go("Auto Assign")}>
              Run AutoAssign <span>›</span>
            </button>
            <button onClick={() => go("Assignments")}>
              Assignment Center <span>›</span>
            </button>
            <button onClick={() => go("Payroll")}>
              Process Payroll <span>›</span>
            </button>
            <button onClick={() => go("Officials")}>
              Manage Officials <span>›</span>
            </button>
          </div>
        </section>
      </div>
      <div className="dashboardLowerGrid">
        <section className="dashboardPanel">
          ...
          <div className="assignmentStatusBody">
            <div
              className="statusDonut"
              style={{
                background: `conic-gradient(#34c759 0 ${pct(fullyAssigned)}%,#ffbf18 ${pct(fullyAssigned)}% ${pct(fullyAssigned) + pct(partial)}%,#ff3b30 ${pct(fullyAssigned) + pct(partial)}% 100%)`,
              }}
            >
              <div>
                <strong>{totalActiveGames}</strong>
        ...
        <section className="dashboardPanel">
          <div className="dashboardPanelHead">
            <h3>♟ &nbsp; Officials Overview</h3>
          </div>
          <div className="officialsOverview">
            <div>
            ...
            <div>
              <strong>{inactiveOfficials}</strong>
              <span>Inactive</span>
            </div>
          </div>
          <button
            className="dashboardWideButton"
            onClick={() => go("Officials")}
          >
            View Officials <span>›</span>
          </button>
        </section>
        <section className="dashboardPanel">
          <div className="dashboardPanelHead">
          ...
          <div className="recentActivity">
            {audit.length ? (
              ...
              <div className="activityEmpty">No recent activity yet.</div>
            )}
          </div>
        </section>
      </div>
      <section className="dashboardPanel" style={{ marginTop: 18 }}>
        <div className="dashboardPanelHead">
          <h3>Action Center</h3>
          <span style={{ fontSize: 12, color: "#617a90" }}>
        ...
        <div className="dashboardActionStrip">
          {cards.map(([key, label]) => (
            <button
              ...
              style={
                focus === key
                  ? { borderColor: "#087af1", background: "#edf6ff" }
                  : undefined
              }
            >
        ...
        {focus === "imports" ? (
          <div className="tableWrap" style={{ margin: 14 }}>
            <table>
              <thead>
                <tr>
                  <th>Time</th>
          ...
          <p style={{ padding: "0 18px 18px", color: "#617a90" }}>
