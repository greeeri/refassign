"use client";
import { FormEvent, useEffect, useState } from "react";
import ReportingAccessManager from "./ReportingAccessManager";
import SuperAdminTaxDocuments from "./SuperAdminTaxDocuments";
import SubscriptionAccessManager from "./SubscriptionAccessManager";
import BillingAlertsManager from "./BillingAlertsManager";
import SuperAdminResourceManager from "./SuperAdminResourceManager";
import LeaguePayrollSettingsManager from "./LeaguePayrollSettingsManager";
import OrganizationRegistrationPaymentsManager from "./OrganizationRegistrationPaymentsManager";
type Access = { user_id: string; program_id: string; role: string };
type Account = {
  id: string;
  email: string;
  full_name: string;
  active: boolean;
  roles: string[];
  program_access: Access[];
  protected: boolean;
  last_login_at: string | null;
};
type Program = { id: string; name: string };
const roleLabels: Record<string, string> = {
  admin: "Administrator",
  assignor: "Assignor",
  league_admin: "League Administrator",
  registrar: "Registrar",
  official: "Official",
  contact: "Contact",
};
function formatLastLogin(value: string | null) {
  if (!value) return "Never";
  return new Intl.DateTimeFormat(undefined, {
    dateStyle: "medium",
    timeStyle: "short",
  }).format(new Date(value));
}
export default function SuperAdminManager() {
  const [accounts, setAccounts] = useState<Account[]>([]),
    [programs, setPrograms] = useState<Program[]>([]),
    [selected, setSelected] = useState<Account | null>(null),
    [role, setRole] = useState("official"),
    [programIds, setProgramIds] = useState<string[]>([]),
    [busy, setBusy] = useState(false),
    [error, setError] = useState(""),
    [notice, setNotice] = useState("");
  async function load() {
    const response = await fetch("/api/super-admin/accounts");
    const result = await response.json();
    if (!response.ok)
      return setError(result.error || "Unable to load accounts.");
    setAccounts(result.accounts);
    setPrograms(result.programs);
  }
  useEffect(() => {
    void load();
  }, []);
  function choose(account: Account) {
    setSelected(account);
    setRole(account.roles[0] || "official");
    setProgramIds(account.program_access.map((a) => a.program_id));
    setError("");
    setNotice("");
  }
  function toggleProgram(id: string) {
    setProgramIds((old) =>
      old.includes(id) ? old.filter((x) => x !== id) : [...old, id],
    );
  }
  async function createAccount(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setBusy(true);
    setError("");
    const form = new FormData(event.currentTarget),
      newRole = String(form.get("role") || "official");
    const response = await fetch("/api/super-admin/accounts", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        email: form.get("email"),
        full_name: form.get("full_name"),
        role: newRole,
        program_ids:
          newRole === "registrar" || newRole === "league_admin"
            ? programIds
            : [],
      }),
    });
    const result = await response.json();
    if (!response.ok) setError(result.error);
    else {
      setNotice("Account created and invitation sent.");
      event.currentTarget.reset();
      setProgramIds([]);
      await load();
    }
    setBusy(false);
  }
  async function save() {
    if (!selected) return;
    setBusy(true);
    setError("");
    const response = await fetch("/api/super-admin/accounts", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        id: selected.id,
        role,
        active: selected.active,
        program_ids: programIds,
      }),
    });
    const result = await response.json();
    if (!response.ok) setError(result.error);
    else {
      setNotice("Account access updated.");
      await load();
    }
    setBusy(false);
  }
  async function remove() {
    if (
      !selected ||
      selected.protected ||
      !window.confirm(`Permanently delete ${selected.email}?`)
    )
      return;
    setBusy(true);
    const response = await fetch("/api/super-admin/accounts", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: selected.id }),
    });
    const result = await response.json();
    if (!response.ok) setError(result.error);
    else {
      setSelected(null);
      setNotice("Account deleted.");
      await load();
    }
    setBusy(false);
  }
  async function supportLink() {
    if (!selected || selected.protected) return;
    setBusy(true);
    const response = await fetch("/api/super-admin/support-link", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ target_user_id: selected.id }),
    });
    const result = await response.json();
    if (!response.ok) setError(result.error);
    else {
      await navigator.clipboard.writeText(result.url);
      setNotice(
        "One-time support sign-in link copied. Open it in a private/incognito window so your super-admin session stays signed in.",
      );
    }
    setBusy(false);
  }
  const needsProgram = role === "registrar" || role === "league_admin";
  return (
    <>
      <section className="card">
        <div className="cardHead">
          <div>
            <h2>Super Admin</h2>
            <p>Private account control and audited technical-support access.</p>
          </div>
          <span className="badge green">Protected Owner</span>
        </div>
        {error && <div className="errorBox">{error}</div>}
        {notice && <div className="loginMessage">{notice}</div>}
        <form className="officialForm" onSubmit={createAccount}>
          <label>
            Name
            <input name="full_name" required />
          </label>
          <label>
            Email
            <input name="email" type="email" required />
          </label>
          <label>
            Account Type
            <select
              name="role"
              defaultValue="official"
              onChange={(e) => setRole(e.target.value)}
            >
              {Object.entries(roleLabels).map(([value, label]) => (
                <option key={value} value={value}>
                  {label}
                </option>
              ))}
            </select>
          </label>
          {needsProgram && (
            <fieldset>
              <legend>Registration Program Access</legend>
              <div className="eligibilityGrid">
                {programs.map((program) => (
                  <label key={program.id}>
                    <input
                      type="checkbox"
                      checked={programIds.includes(program.id)}
                      onChange={() => toggleProgram(program.id)}
                    />
                    {program.name}
                  </label>
                ))}
              </div>
            </fieldset>
          )}
          <button className="primary" disabled={busy}>
            Create Account & Send Invitation
          </button>
        </form>
      </section>
      <section className="card">
        <h2>Account Management</h2>
        <div className="tableWrap">
          <table>
            <thead>
              <tr>
                <th>Account</th>
                <th>Type</th>
                <th>Registration Access</th>
                <th>Last Login</th>
                <th>Status</th>
              </tr>
            </thead>
            <tbody>
              {accounts.map((a) => (
                <tr
                  key={a.id}
                  onClick={() => choose(a)}
                  className={selected?.id === a.id ? "selectedRow" : ""}
                >
                  <td>
                    <b>{a.full_name || a.email}</b>
                    <small>{a.email}</small>
                  </td>
                  <td>
                    {a.protected
                      ? "Protected Owner"
                      : a.roles.map((r) => roleLabels[r] || r).join(", ")}
                  </td>
                  <td>
                    {a.program_access
                      .map(
                        (x) =>
                          programs.find((p) => p.id === x.program_id)?.name,
                      )
                      .filter(Boolean)
                      .join(", ") || "—"}
                  </td>
                  <td>
                    <time dateTime={a.last_login_at || undefined}>
                      {formatLastLogin(a.last_login_at)}
                    </time>
                  </td>
                  <td>{a.active ? "Active" : "Inactive"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        {selected && (
          <div className="registrationReview">
            <h3>{selected.full_name || selected.email}</h3>
            {selected.protected ? (
              <p>
                This owner account is locked. No other account can edit, delete,
                or impersonate it.
              </p>
            ) : (
              <>
                <label>
                  Account Type
                  <select
                    value={role}
                    onChange={(e) => setRole(e.target.value)}
                  >
                    {Object.entries(roleLabels).map(([value, label]) => (
                      <option key={value} value={value}>
                        {label}
                      </option>
                    ))}
                  </select>
                </label>
                {needsProgram && (
                  <fieldset>
                    <legend>Registration Program Access</legend>
                    <div className="eligibilityGrid">
                      {programs.map((program) => (
                        <label key={program.id}>
                          <input
                            type="checkbox"
                            checked={programIds.includes(program.id)}
                            onChange={() => toggleProgram(program.id)}
                          />
                          {program.name}
                        </label>
                      ))}
                    </div>
                  </fieldset>
                )}
                <div className="toolbar">
                  <button
                    className="primary"
                    disabled={busy}
                    onClick={() => void save()}
                  >
                    Save Access
                  </button>
                  <button
                    className="secondary"
                    disabled={busy}
                    onClick={() => void supportLink()}
                  >
                    Copy One-Time Support Login
                  </button>
                  <button
                    className="danger"
                    disabled={busy}
                    onClick={() => void remove()}
                  >
                    Delete Account
                  </button>
                </div>
              </>
            )}
          </div>
        )}
      </section>
      <ReportingAccessManager />
      <LeaguePayrollSettingsManager />
      <OrganizationRegistrationPaymentsManager />
      <SubscriptionAccessManager />
      <BillingAlertsManager />
      <SuperAdminResourceManager />
      <SuperAdminTaxDocuments />
    </>
  );
}
