...
const roles=["admin","assignor","viewer","mentor_viewer","billing"] as const;
const labels:Record<string,string>={owner:"Organization owner",admin:"Administrator",assignor:"Assignor",mentor_viewer:"Mentor — assignments only (read-only)",viewer:"Contact (read-only)",billing:"Billing manager"};
const permissions=[
...
export default function OrganizationTeamManager({organizationId,organization,canManage,leagues}:{organizationId:string;organization:string;canManage:boolean;leagues:LeagueChoice[]}){
 ...
 const needsLeagues=role==="assignor"||role==="viewer"||role==="mentor_viewer";
 const invite=async()=>{
  ...
  const supabase=createClient();
  const {data,error}=await supabase.functions.invoke("send-organization-invitation",{body:{organizationId,email,role:role==="mentor_viewer"?"viewer":role,viewerPermissions:role==="mentor_viewer"?["assignments"]:role==="viewer"?viewerPermissions:[],leagueIds:needsLeagues?validLeagueIds:[]}});
  if(error){
 ...
 return <div className={styles.wrap}>
  ...
  <section className={styles.list}><div className={styles.listHead}><h3>Organization access</h3><b>{team.members.length} active · {team.invitations.length} pending</b></div>{[...team.members,...team.invitations].length===0?<p className={styles.empty}>No additional team members have been added.</p>:[...team.members,...team.invitations].map(item=><MemberCard key={item.id} item={item} leagues={validLeagues} disabled={!canManage||busy===item.id} onSave={save}/>)}</section>
 </div>
}
...
const displayRole=(item:TeamAccess)=>item.role==="viewer"&&item.viewer_permissions?.length===1&&item.viewer_permissions[0]==="assignments"?"mentor_viewer":item.role;

function MemberCard({item,leagues,disabled,onSave}:{item:TeamAccess;leagues:LeagueChoice[];disabled:boolean;onSave:(item:TeamAccess,role:string,permissions:string[],leagueIds:string[])=>Promise<void>}){const [role,setRole]=useState(displayRole(item)),[selected,setSelected]=useState(item.viewer_permissions||[]),[leagueIds,setLeagueIds]=useState(item.league_ids||[]);useEffect(()=>{setRole(displayRole(item));setSelected(item.viewer_permissions||[]);setLeagueIds(item.league_ids||[])},[item.role,item.viewer_permissions,item.league_ids]);const locked=item.role==="owner",needsLeagues=role==="assignor"||role==="viewer"||role==="mentor_viewer";return <article className={styles.member}><div className={styles.identity}><span>{item.email.slice(0,1).toUpperCase()}</span><div><strong>{item.email}</strong><small>{labels[displayRole(item)]||item.role}</small></div><em>{item.status==="pending"?"Pending":"Active"}</em></div>{locked?<p>The organization owner role cannot be changed here.</p>:<><label>Role<select value={role} disabled={disabled} onChange={e=>setRole(e.target.value)}>{roles.map(code=><option value={code} key={code}>{labels[code]}</option>)}</select></label>{needsLeagues&&<LeaguePicker leagues={leagues} selected={leagueIds} onChange={setLeagueIds}/>} {role==="viewer"&&<PermissionPicker selected={selected} onChange={setSelected}/>}<button className="primary" disabled={disabled||needsLeagues&&!leagueIds.length||role==="viewer"&&!selected.length} onClick={()=>void onSave(item,role,selected,leagueIds)}>{disabled?"Saving…":"Save changes"}</button></>}</article>}
