...
export async function POST(req: NextRequest) {
  ...
  const body = (await req.json().catch(() => ({}))) as {
    ...
    channel?: "email" | "text";
    messageType?: keyof typeof labels;
    note?: string;
  };
  const ids = [...new Set(body.assignmentIds || [])].slice(0, 100),
    channel = body.channel,
    messageType = body.messageType;
  if (!ids.length || !channel || !messageType || !labels[messageType])
  ...
  const { data: rows, error } = await supabase
    .from("assignments")
    .select(
      "id,game_id,official_id,status,response_token,officials(first_name,last_name,email,phone),sport_positions(name),games!inner(game_number,starts_at,status,organization_id,home:teams!games_home_team_id_fkey(name),away:teams!games_away_team_id_fkey(name),location:locations(name,address,city,state))",
    )
    .in("id", ids)
  ...
  for (const raw of rows || []) {
    const a = raw as any,
      o = Array.isArray(a.officials) ? a.officials[0] : a.officials,
      g = Array.isArray(a.games) ? a.games[0] : a.games,
      p = Array.isArray(a.sport_positions)
        ? a.sport_positions[0]
        : a.sport_positions;
    const recipient = channel === "email" ? o?.email : o?.phone,
      home = (Array.isArray(g?.home) ? g.home[0] : g?.home)?.name || "TBD",
    ...
    const subject = `${labels[messageType]}: ${home} vs ${away}`,
      link = a.response_token
        ? `${req.nextUrl.origin}/assignment/${a.response_token}`
        : req.nextUrl.origin;
    ...
    const { data: log, error: logError } = await supabase
      ...
      .single();
    if (logError) {
      failures.push(logError.message);
      continue;
    }
    if (!recipient) {
      const problem = `${o?.first_name || "Official"}: missing ${channel === "email" ? "email" : "mobile number"}`;
      failures.push(problem);
      await supabase
        .from("official_communications")
        .update({ delivery_status: "failed", error_message: problem })
        .eq("id", log.id);
      continue;
    }
    ...
    if (channel === "email") {
      ...
      if (!sid || !token || !from)
        problem = "Text messaging is not configured yet.";
      else {
    ...
    if (ok) sent++;
      ...
      failures.push(
        `${o.first_name} ${o.last_name}: ${problem || "send failed"}`,
      );
  }
