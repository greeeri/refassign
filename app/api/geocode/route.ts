...
export async function GET(request: NextRequest) {
  ...
  const venueName = request.nextUrl.searchParams.get("venueName")?.trim();
  const isVenue = request.nextUrl.searchParams.get("type") === "venue";
  if (
    (!isVenue && !address) ||
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "The address could not be located right now." },
      { status: 422 },
    );
  }
}
