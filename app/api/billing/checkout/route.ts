...
export async function POST(request:NextRequest){
 ...
 if(purchaserState==="IA"&&!savedExemption&&exemptionCertificate){
  ...
  await service.from("organization_tax_exemption_documents").update({status:"superseded",superseded_at:new Date().toISOString()}).eq("organization_id",pending.organization_id).is("superseded_at",null);
  const{error:historyError}=await service.from("organization_tax_exemption_documents").insert({organization_id:pending.organization_id,subscription_id:pendingId,storage_path:storagePath,original_name:exemptionCertificate.name,mime_type:exemptionCertificate.type,file_size_bytes:exemptionCertificate.size,status:"submitted",submitted_by:user.id});
