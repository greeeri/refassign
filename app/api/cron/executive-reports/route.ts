...
export async function GET(request: NextRequest) {
  ...
  for (const schedule of schedules || []) {
    try {
      ...
      const html = `<div style="font-family:Arial,sans-serif;background:#f5f7fb;padding:28px"><div style="max-width:680px;margin:auto;background:white;border:1px solid #e2e8f0;border-radius:14px;overflow:hidden"><div style="background:#14233b;color:white;padding:24px 28px"><h2 style="margin:0">${esc(summary.organization)} Executive Summary</h2><p style="margin:8px 0 0;color:#cbd5e1">${esc(schedule.frequency === "monthly" ? "Monthly" : "Weekly")} Ref Pro Group report</p></div><div style="padding:28px"><h3>Needs attention</h3><p>${esc(attention)}</p><table style="width:100%;border-collapse:collapse"><tr><td style="padding:10px;border-bottom:1px solid #e2e8f0">Upcoming games</td><td><b>${summary.upcomingGames}</b></td><td>Coverage</td><td><b>${summary.coverageRate}%</b></td></tr><tr><td style="padding:10px;border-bottom:1px solid #e2e8f0">Staffing risks</td><td><b>${summary.staffingRisks}</b></td><td>Outstanding payroll</td><td><b>${money(summary.outstandingPayroll)}</b></td></tr><tr><td style="padding:10px">Compliance issues</td><td><b>${summary.complianceIssues}</b></td><td>Training participation</td><td><b>${summary.trainingRate}%</b></td></tr></table><p style="margin-top:24px"><a href="${request.nextUrl.origin}/workspace?section=reports" style="background:#2563eb;color:white;text-decoration:none;padding:11px 17px;border-radius:8px;font-weight:700">Open Executive Dashboard</a></p></div></div></div>`;
      const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}`, "Content-Type": "application/json", "Idempotency-Key": `executive-${schedule.id}-${now.slice(0, 10)}` }, body: JSON.stringify({ from: "Ref Pro Group <notifications@assignments.ref-assign.com>", to: [schedule.recipient_email], reply_to: "assignments@ref-assign.com", subject: `${summary.organization} — Ref Pro Group Executive Summary`, html, attachments: [{ filename: "refassign-executive-summary.pdf", content: attachment }] }) });
