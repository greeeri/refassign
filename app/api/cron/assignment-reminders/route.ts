...
export async function GET(req:NextRequest){
  ...
  for(const raw of rows||[]){
    ...
    const response=await sendOfficialNotification(supabase, assignment.official_id, "https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json","Idempotency-Key":`automatic-reminder-${assignment.id}-${game.starts_at}`},body:JSON.stringify({from:"Ref Pro Group <notifications@assignments.ref-assign.com>",to:[official.email],reply_to:"assignments@ref-assign.com",subject:`Game Tomorrow: ${home} vs ${away}`,html})});
    if(response.ok){
      sent++;
