...
export async function GET(request: NextRequest) {
  ...
  const { data: officialLinks, error: officialLinksError } = await service
    .from("organization_officials")
    .select("official_id")
    .eq("organization_id", organizationId)
    .eq("active", true);
  if (officialLinksError)
    return NextResponse.json(
      { error: officialLinksError.message },
      { status: 400 },
    );
  const { data: officials, error: officialsError } = await service
    .from("officials")
  ...
  if (gameIds.length) {
    const { data, error } = await supabase
      .from("audit_history")
      .select("id,game_id,assignment_id,action,occurred_at,old_data,new_data")
      .in("game_id", gameIds)
      .in("action", [
        "assigned",
        "assignment_changed",
  ...
  const declines = [...historicalDeclines, ...currentDeclines].map(
    (decline) => {
      const replacementEvents = history.flatMap((event) => {
        if (
          event.game_id !== decline.game_id ||
          new Date(String(event.occurred_at)).getTime() <=
            new Date(String(decline.declined_at)).getTime()
        )
          return [];
        const next =
          event.new_data && typeof event.new_data === "object"
            ...
            : {};
        if (
          text(next.position_id) !== decline.position_id ||
          text(next.official_id) === decline.official_id ||
        ...
        return [
          {
            official_id: text(next.official_id),
            replaced_at: text(event.occurred_at),
          },
        ];
      });
      const currentReplacement = assignments.find(
        (row) =>
          row.game_id === decline.game_id &&
          row.position_id === decline.position_id &&
          row.official_id !== decline.official_id &&
          row.status !== "declined" &&
      ...
      const replacement =
        ...
        (currentReplacement
          ? {
              official_id: currentReplacement.official_id,
              replaced_at: currentReplacement.assigned_at,
            }
          : null);
      return {
        ...decline,
        replacement_official_id: replacement?.official_id || null,
        replaced_at: replacement?.replaced_at || null,
      };
    },
  );
