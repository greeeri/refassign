...
async function context(request: NextRequest) {
  ...
  const { session: supabase, service, user, organizationId } = scope;
  ...
  (subscriptions || []).forEach((row) => {
    if (row.organization_id && !latest.has(row.organization_id))
      latest.set(row.organization_id, row.reporting_access);
  });
  const premiumOrganizationIds = organizationIds.filter(
    (id) => latest.get(id) !== "standard",
  );
  return {
    supabase,
    service,
    user,
    organizationIds: premiumOrganizationIds,
    premium: premiumOrganizationIds.length > 0,
  };
}
export async function GET(request: NextRequest) {
  ...
  const [organizationsResult, gamesResult, assignmentsResult, templatesResult] =
    await Promise.all([
      ...
      organizationIds.length
        ? service
            ...
            .select(
              "id,organization_id,name,definition,recipient_email,frequency,send_day,schedule_enabled,last_sent_at,next_send_at,updated_at",
            )
            .eq("user_id", user.id)
            .in("organization_id", organizationIds)
...
export async function POST(request: NextRequest) {
  ...
  if (!premium)
    return NextResponse.json(
      { error: "Premium Reporting is required for the Custom Report Builder." },
      { status: 403 },
    );
  ...
  if (!name || !definition || JSON.stringify(definition).length > 20000)
    return NextResponse.json(
      { error: "Enter a report name and valid report configuration." },
      { status: 400 },
    );
  const requested = Array.isArray(definition.organizationIds)
      ? definition.organizationIds.filter((id: string) =>
          organizationIds.includes(id),
        )
      : [],
    organizationId = requested[0] || organizationIds[0];
  if (!organizationId)
    return NextResponse.json(
      { error: "A managed organization is required." },
      { status: 403 },
    );
  const frequency = body.frequency === "monthly" ? "monthly" : "weekly",
    ...
    sendDay = Math.min(
      max,
      Math.max(min, Math.round(Number(body.sendDay ?? 1))),
    ),
    recipient = String(body.recipientEmail || user.email || "")
      .trim()
      .slice(0, 320),
    enabled = Boolean(body.scheduleEnabled);
  if (enabled && !/^\S+@\S+\.\S+$/.test(recipient))
    return NextResponse.json(
      { error: "Enter a valid delivery email." },
      { status: 400 },
    );
  const payload = {
    ...
    user_id: user.id,
    name,
    definition: { ...definition, organizationIds: requested },
    recipient_email: recipient || null,
    frequency,
    send_day: sendDay,
    schedule_enabled: enabled,
    next_send_at: enabled ? nextSend(frequency, sendDay) : null,
  ...
  let query = body.id
    ? service
        ...
        .eq("user_id", user.id)
        .in("organization_id", organizationIds)
    : service.from("custom_report_templates").insert(payload);
  ...
  return error
    ? NextResponse.json({ error: error.message }, { status: 400 })
    : NextResponse.json({ template: data });
}
