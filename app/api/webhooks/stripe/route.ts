...
export async function POST(request: Request) {
  ...
  const { data: registration, error } = await supabase
    ...
    .select(
      "id,first_name,last_name,email,registration_year,registrar_notified_at",
    )
    .single();
  if (error)
    ...
    if (unique.length) {
      const emailResponse = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          html: `<div style="font-family:Arial,sans-serif"><h2>New Official Registration</h2><p><b>${escapeHtml(registration.first_name)} ${escapeHtml(registration.last_name)}</b> has completed the ${escapeHtml(registration.registration_year)} registration payment.</p><p>${escapeHtml(registration.email)}</p><p>Sign in to RefAssign and open Registrar to create level eligibility.</p></div>`,
        }),
      });
      if (emailResponse.ok)
        await supabase
