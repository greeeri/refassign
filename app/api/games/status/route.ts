...
export async function POST(req: NextRequest) {
  ...
  if (!body.gameId || !body.status)
    return NextResponse.json(
      { error: "Game and status are required." },
      { status: 400 },
    );
  ...
  if (!["canceled", "rained_out"].includes(body.status))
    return NextResponse.json({
      updated: true,
      sent: 0,
      failed: 0,
  ...
  if (!apiKey)
    return NextResponse.json({
      ...
      sent: 0,
      failed: (rows || []).length,
      failures: ["RESEND_API_KEY is not configured."],
    });
  let sent = 0;
  ...
  for (const raw of rows || []) {
    const a = raw as any,
      ...
      pos = Array.isArray(a.sport_positions)
        ...
        : a.sport_positions;
    if (!o?.email) {
      const message = `${o?.first_name || "Official"}: missing email`;
      failures.push(message);
      await service
        .from("assignments")
        .update({ cancellation_email_error: message })
        .eq("id", a.id);
      continue;
    }
    ...
    if (response.ok) {
      ...
      await service
        ...
        .update({
          ...
          cancellation_email_id: result.id || null,
