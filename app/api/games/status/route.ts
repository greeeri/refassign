...
export async function POST(req:NextRequest){
  ...
  for(const raw of rows||[]){
    ...
    if(!o?.email){const message=`${o?.first_name||'Official'}: missing email`;failures.push(message);await supabase.from('assignments').update({cancellation_email_error:message}).eq('id',a.id);continue}
    ...
    if(response.ok){sent++;await supabase.from('assignments').update({cancellation_notified_at:new Date().toISOString(),cancellation_email_id:result.id||null,cancellation_email_error:null}).eq('id',a.id)}else{const message=result.message||`Email provider returned ${response.status}`;failures.push(`${o.first_name} ${o.last_name}: ${message}`);await supabase.from('assignments').update({cancellation_email_error:message}).eq('id',a.id)}
  }
