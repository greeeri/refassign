...
export async function POST(req:NextRequest){
  ...
  for(const raw of rows||[]){
    ...
    const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json','Idempotency-Key':`assignment-${a.id}-${a.published_at}`},body:JSON.stringify({from:'RefAssign <assignments@ref-assign.com>',to:[o.email],reply_to:'assignments@ref-assign.com',subject:`Game Assignment: ${home} vs ${away}`,html})})
