...
export async function POST(req: NextRequest) {
  ...
  for (const raw of rows || []) {
    ...
    const deadline = a.accept_by
      ? new Date(a.accept_by).toLocaleString("en-US", {
          timeZone: "America/Chicago",
          month: "short",
          day: "numeric",
          hour: "numeric",
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      headers: {
        ...
        "Idempotency-Key": `assignment-${a.id}-${a.published_at}`,
      },
      body: JSON.stringify({
    ...
    if (response.ok) {
      ...
      const msg = result.message || `Resend returned ${response.status}`;
      failures.push(`${o.first_name} ${o.last_name}: ${msg}`);
      await service
        .from("assignments")
