...
export async function POST(req:NextRequest){
  ...
  if(!((roles||[]) as string[]).some(role=>['admin','assignor'].includes(role)))return NextResponse.json({error:'Only Administrators and Assignors can publish assignments.'},{status:403})
  ...
  for(const raw of rows||[]){
    ...
    if(response.ok){sent++;await service.from('assignments').update({email_sent_at:new Date().toISOString(),resend_email_id:result.id||null,email_error:null}).eq('id',a.id)}else{const msg=result.message||`Resend returned ${response.status}`;failures.push(`${o.first_name} ${o.last_name}: ${msg}`);await service.from('assignments').update({email_error:msg}).eq('id',a.id)}
  }
