...
export default function TestAuthPanel({onAuthenticated}:{onAuthenticated:(email:string)=>void}){
 ...
 useEffect(()=>{if(linkCooldown<=0)return;const timer=window.setInterval(()=>setLinkCooldown(value=>Math.max(0,value-1)),1000);return()=>window.clearInterval(timer)},[linkCooldown]);
 const submit=async(event:FormEvent)=>{event.preventDefault();setLoading(true);setMessage("");const supabase=createTierTestClient();
  if(mode==="signup"){
   const {data,error}=await supabase.auth.signUp({email,password,options:{data:{full_name:name},emailRedirectTo:`${window.location.origin}/workspace`}});
 ...
 const emailLink=async()=>{if(!email.includes("@")||linkCooldown>0)return;setLoading(true);setMessage("");const supabase=createTierTestClient();const {error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:`${window.location.origin}/workspace`,shouldCreateUser:true}});if(error){const rateLimited=error.message.toLowerCase().includes("rate limit");setMessage(rateLimited?"The test email service has reached its sending limit. Please wait before trying again, or sign in with your password if your account is already confirmed.":error.message);if(rateLimited)setLinkCooldown(60)}else{setMessage("Sign-in email sent. Open the secure link in that email to continue to your workspace.");setLinkCooldown(60)}setLoading(false)};
 return <div className={auth.panel}><div className={auth.heading}><div><span>Secure test account required</span><h4>{mode==="signup"?"Create your organization-owner account":"Sign in to save this workspace"}</h4></div><div><button className={mode==="signup"?auth.active:""} onClick={()=>setMode("signup")} type="button">Create account</button><button className={mode==="signin"?auth.active:""} onClick={()=>setMode("signin")} type="button">Sign in</button></div></div><form onSubmit={submit}>{mode==="signup"&&<label>Your name<input required value={name} onChange={e=>setName(e.target.value)} autoComplete="name"/></label>}<label>Email address<input required type="email" value={email} onChange={e=>setEmail(e.target.value)} autoComplete="email"/></label><label>Password<input required minLength={8} type="password" value={password} onChange={e=>setPassword(e.target.value)} autoComplete={mode==="signup"?"new-password":"current-password"}/></label><button disabled={loading}>{loading?"Please wait…":mode==="signup"?"Create secure test account":"Sign in"}</button></form>{mode==="signin"&&<button className={auth.magic} disabled={loading||!email.includes("@")||linkCooldown>0} onClick={()=>void emailLink()} type="button">{linkCooldown>0?`Request another link in ${linkCooldown}s`:"Email me a secure sign-in link"}</button>}{message&&<p>{message}</p>}<small>This account exists only in the separate RefAssign test database.</small></div>
}
