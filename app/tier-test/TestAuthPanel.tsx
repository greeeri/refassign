...
export default function TestAuthPanel({onAuthenticated}:{onAuthenticated:(email:string)=>void}){
 ...
 const emailLink=async()=>{if(!email.includes("@")||linkCooldown>0)return;setLoading(true);setMessage("");const supabase=createTierTestClient();const {error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:`${window.location.origin}/tier-test`,shouldCreateUser:true}});if(error){const rateLimited=error.message.toLowerCase().includes("rate limit");setMessage(rateLimited?"The test email service has reached its sending limit. Please wait before trying again, or sign in with your password if your account is already confirmed.":error.message);if(rateLimited)setLinkCooldown(60)}else{setMessage("Sign-in email sent. Open the secure link in that email to continue.");setLinkCooldown(60)}setLoading(false)};
