...
export default function Home(){
 ...
 const stats=useMemo(()=>({
   games:games.length,
   need:games.filter(g=>g.assigned===0).length,
   partial:games.filter(g=>g.assigned>0&&g.assigned<g.needed).length,
   full:games.filter(g=>g.assigned>=g.needed).length
 }),[games])
 ...
 return <div className="shell">
   <aside><div className="brand">Ref<span>Assign</span></div><div className="tag">OFFICIALS MANAGEMENT</div><nav>{nav.map(n=><button key={n} className={section===n?'active':''} onClick={()=>setSection(n)}>{n}</button>)}</nav><div className="asideFoot">Built for soccer.<br/>Ready for every sport.</div></aside>
   <main>
    ...
    {section==='Assignments' && <><div className="actionbar"><div><h2>Assignment Center</h2><p>Fill open positions manually or let RefAssign propose a crew.</p></div><button className="success" onClick={autoAssign}>⚡ Auto Assign Open Games</button></div>{games.map(g=><section className="card assignment" key={g.id}><div><h3>{g.home} vs {g.away}</h3><p>{g.date} • {g.time} • {g.sport} • {g.location}</p></div><div className="fill"><b>{g.assigned}/{g.needed}</b><span>positions filled</span></div>{badge(g)}</section>)}</>}
    ...
    {section==='Sports & Rules' && <section className="card"><h2>Sport Assignment Rules</h2><Rule sport="Soccer" count="3 default" roles="Center Referee • AR1 • AR2 • optional 4th"/><Rule sport="Football" count="5–7" roles="Referee • Umpire • Lines • Back/Field/Side Judges"/><Rule sport="Basketball" count="2–3" roles="Crew Chief • Referees"/><Rule sport="Baseball / Softball" count="2–4" roles="Plate Umpire • Base Umpires"/><Rule sport="Volleyball" count="2–4" roles="R1 • R2 • Line Judges"/></section>}
   </main>
 </div>
}
