...
export default function BillingPage(){
 ...
 useEffect(()=>{const params=new URLSearchParams(window.location.search);const requested=params.get("plan");const isNewOrganization=params.get("new")==="1";if(plans.some(item=>item.key===requested))setPlan(requested as PlanKey);void(async()=>{const s=createClient();const{data:{user}}=await s.auth.getUser();setUserEmail(user?.email||"");setAuthReady(true);if(!user||isNewOrganization)return;const[{data},taxResponse]=await Promise.all([s.from("refassign_subscriptions").select("organization_name,plan,status,trial_ends_at,current_period_end,founding_offer").eq("user_id",user.id).order("created_at",{ascending:false}).limit(1).maybeSingle(),fetch("/api/billing/tax-exemption")]);const subscription=data as SubscriptionSummary|null;setSub(subscription);if(subscription?.organization_name)setOrg(subscription.organization_name);if(subscription&&plans.some(item=>item.key===subscription.plan))setPlan(subscription.plan as PlanKey);if(taxResponse.ok){const tax=await taxResponse.json();setTaxExemption(tax.exemption||null);if(tax.exemption){setIowaCertified(true);setState(tax.exemption.purchaser_state||"IA")}}})()},[]);
 ...
 const returnPath=useMemo(()=>`/billing?new=1&plan=${plan}`,[plan]);
 ...
 async function checkout(){
  setMessage("");
  if(!org.trim()){setMessage("Enter the new organization name before continuing.");return}
  if(officials>250){setMessage("Organizations with more than 250 officials require an Enterprise plan.");return}
  if(state==="IA"&&!taxExemption){setMessage("Save the Iowa exemption certificate before continuing to payment.");return}
  setBusy("checkout");
  try{const form=new FormData();form.set("plan",plan);form.set("organization_name",org);form.set("official_count",String(officials));form.set("texting_addon",String(texting));form.set("purchaser_state",state);form.set("iowa_exemption_certified",String(iowaCertified));if(certificate)form.set("exemption_certificate",certificate);if(taxExemption?.subscription_id)form.set("prepared_subscription_id",taxExemption.subscription_id);const r=await fetch("/api/billing/checkout",{method:"POST",body:form});const j=await r.json().catch(()=>({error:"The checkout service returned an unreadable response."}));if(r.ok&&j.url)location.assign(j.url);else setMessage(j.error||"Could not start checkout.")}
  catch{setMessage("Could not reach the secure payment service. Please try again.")}
  finally{setBusy("")}
 }
